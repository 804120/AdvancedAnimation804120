function FlockSystem(color, num, ctx){
  this.color = color;
  this.context = ctx;
  this.boids = [];
  ...
  for(let i=0;i<num;i++){
    let randompos = new JSVector(Math.random()*canvas.width, Math.random()*canvas.height);
    let randomvel = new JSVector(Math.random()-0.5, Math.random()-0.5);
    this.boids.push(new Boid(randompos, randomvel.setMagnitude(3), new JSVector(0, 0), 15, this.color, this.context));
  }
}
